...
describe("Hello World", () => {
  ...
  it("calculPercentage", () => {
    ...
    const total = 60;
    const offer = { type: "percentage", value: 10 };
  ...
  it("calculSlide", () => {
    ...
    const total = 60;
    const offer = { type: "percentage", value: 10, sliceValue: 50 };
    expect(calculSlide(total, offer)).toEqual(50);
  });

  it("getMin", () => {
    const calcul = [4, 2, 6];
    expect(getMin(calcul)).toEqual(2);
  });
  ...
  it("getResultOffer", () => {
    ...
    let result = [50, 58.5, 65];

    expect(getResultOffer(offers, cart)).toEqual(result);
  });
});
